...
module.exports = {
    ...
    storage_write_local_storage: function(userID,valueName,value) {
        return new Promise((resolve, reject)=>{
            try{
                var readStorage = this.storage_read_db();
    ...
    storage_write_jackpot: function(valueName,value) {
        return new Promise((resolve, reject)=>{
            try{
                var readStorage = this.storage_read_db();
                var storage_write = db.set('jackpot.'+valueName, value).write();
                resolve(true);
            }catch (error){
                var errorMessage = "storage_write_jackpot: Can't write to local storage.";
                if(config.bot.errorLogging){
                    log.log_write_file(errorMessage);
                    log.log_write_file(error);
                }
    ...
    storage_read_db: function() {
            try{
                 db.read();
            }catch (error){
                var errorMessage = "storage_read_db: Can't read local storage.";
                if(config.bot.errorLogging){
                    log.log_write_file(errorMessage);
                    log.log_write_file(error);
                }
    ...
    storage_read_all: function() {
        return new Promise((resolve, reject)=>{
            try{ 
                var content = fs.readFileSync(process.cwd()+'/lowdb/lowdb.json');
                var jsonContent = JSON.parse(content);
                resolve(jsonContent);
            }catch (error){
                var errorMessage = "storage_read_all: Can't read total local storage.";
                if(config.bot.errorLogging){
                    log.log_write_file(errorMessage);
                    log.log_write_file(error);
                }
    ...
    storage_read_jackpot: function() {    
        return new Promise((resolve, reject)=>{
            try{
                var readStorage = this.storage_read_db();
                var storage_read = db.get('jackpot').value();
                resolve(storage_read);
            }catch (error){
                var errorMessage = "storage_read_jackpot: Can't read from local storage.";
                if(config.bot.errorLogging){
                    log.log_write_file(errorMessage);
                    log.log_write_file(error);
                }
